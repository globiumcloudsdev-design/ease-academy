...
export default function ParentsPage() {
  ...
  const loadParents = async () => {
    try {
      ...
      const [allResponse, pendingResponse] = await Promise.all([
        apiClient.get(API_ENDPOINTS.SUPER_ADMIN.PARENTS),
        apiClient.get(API_ENDPOINTS.SUPER_ADMIN.PENDING_PARENTS)
      ]);
      if (allResponse) {
  ...
  const handleCheckChildrenMatches = async (parentId) => {
    try {
      ...
      const response = await apiClient.post(API_ENDPOINTS.SUPER_ADMIN.CHECK_CHILDREN_MATCHES, {
        parentId
      });
                                                            ...
                                                            <div key={student._id} className="flex items-start space-x-2 text-xs bg-white dark:bg-gray-700 p-2 rounded">
                                                              ...
                                                              <div className="flex-1">
                                                                ...
                                                                <div className="text-gray-600 dark:text-gray-400">
                                                                  ...
                                                                  {student.branchName && ` | Branch: ${student.branchName}`}
                                                                </div>
