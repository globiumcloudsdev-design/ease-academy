...
// GET /api/super-admin/fee-vouchers - list vouchers (supports branch filter)
export const GET = withAuth(async (request, user, userDoc) => {
  try {
    ...
    if (search) query.$or = [{ voucherNumber: { $regex: search, $options: 'i' } }];
    ...
    return NextResponse.json({ success: true, data: { vouchers, pagination: { page, limit, total, pages: Math.ceil(total / limit) } } });
  } catch (error) {
    console.error('Error fetching super-admin fee vouchers:', error);
    return NextResponse.json({ success: false, message: error.message || 'Failed to fetch fee vouchers' }, { status: 500 });
  }
}, [requireRole(['super_admin'])]);

// POST /api/super-admin/fee-vouchers - generate vouchers for a branch (intelligent auto-selection)
export const POST = withAuth(async (request, user, userDoc) => {
  try {
    await connectDB();
    const body = await request.json();
    const { branchId, templateId, dueDate, month, year, remarks } = body;

    if (!branchId || !templateId) {
      return NextResponse.json({ success: false, message: 'Branch and template are required' }, { status: 400 });
    }
    if (!dueDate || !month || !year) {
      return NextResponse.json({ success: false, message: 'Due date, month, and year are required' }, { status: 400 });
    }
    ...
    if (template.applicableTo === 'class-specific' && template.classes && template.classes.length > 0) {
      ...
      // This should be handled differently - maybe stored in template
      // For now, return error as we need specific student IDs
      return NextResponse.json({ success: false, message: 'Student-specific templates require manual student selection' }, { status: 400 });
    }
    // else applicableTo === 'all' - all active students in branch

    const students = await User.find(studentQuery).lean();
    if (students.length === 0) return NextResponse.json({ success: false, message: 'No eligible students found' }, { status: 400 });
    ...
    for (const student of students) {
      ...
      try {
        ...
        if (existing) {
          errors.push({ studentId: student._id, studentName, message: 'Voucher already exists for this month' });
          continue;
        }
        ...
        const counter = await Counter.findOneAndUpdate({ _id: 'feeVoucher' }, { $inc: { seq: 1 } }, { new: true, upsert: true });
        ...
        const discountAmount = template.discount?.enabled 
          ? (template.discount.type === 'percentage' ? (baseAmount * template.discount.amount) / 100 : template.discount.amount)
          : 0;
        ...
        if (parentEmail) {
          ...
          // Also create notification for parent if they have account
        ...
        console.error(`Error generating voucher for student ${student._id}:`, err);
        errors.push({ studentId: student._id, studentName, message: err.message });
      }
    }
    ...
    return NextResponse.json({ 
      ...
      data: { vouchers: createdVouchers, errors: errors.length ? errors : undefined } 
    });
  } catch (error) {
    console.error('Error generating super-admin fee vouchers:', error);
    return NextResponse.json({ success: false, message: error.message || 'Failed to generate fee vouchers' }, { status: 500 });
  }
}, [requireRole(['super_admin'])]);
